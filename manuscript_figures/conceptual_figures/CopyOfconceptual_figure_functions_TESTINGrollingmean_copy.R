...
makeFigure1site <- function(sitename, label=NULL, repsitenames = NULL,
                            rolling.mean = TRUE, rolling.mean.k = 7,
                            upper = 0.9, lower = 0.1) {
  ...
  #------ Make plot  layout ------
  #1st and 6th row are buffer (ecoregion title and x axis label) so that panels can be the same height
  ...
  lines(x = SWA_rollMeanyear$Group.1, y = SWA_rollpercentiles[,1], lty = 3, col = SWA_COLOR, lwd = 1)
  lines(x = SWA_rollMeanyear$Group.1, y = SWA_rollpercentiles[,2], lty = 3, col = SWA_COLOR, lwd = 1)
  ...
  #lines(AET_percentiles[,1]*10 ~ xValues, col=alpha("white", 0.1), lwd=10, lty = 1)
  #lines(AET_percentiles[,2]*10 ~ xValues, col=alpha("white", 0.1), lwd=10, lty = 1)
